...
def load_musicgen():
    """
    MusicGen-small local generation.
    This follows the previously working app implementation.
    """
...
def generate_music(prompt: str, max_new_tokens: int = 64, guidance_scale: float = 2.0):
    ...
    This replaces the HF Router API call with the previously working local MusicGen method.
    """
...
if generate_btn and uploaded:
    ...
    # ── MUSIC GENERATION: MusicGen (local, previous working implementation) ─────────────
    st.markdown(" ")
    st.markdown('<p class="step-label">Music Generation — MusicGen-small (local)</p>',
                unsafe_allow_html=True)

    with st.spinner("Composing your music clip with facebook/musicgen-small..."):
        try:
